...
export function createApp(operations, config) {
  ...
  app.get('/api/health', route(() => operations.health()));
  ...
  app.get('/api/status', route(async () => ({ downloads: await operations.history() })));
  app.get('/api/downloads', route(async () => ({ downloads: await operations.history() })));
  app.get('/api/queue', route(async () => ({ items: await operations.list('queue') })));
  app.post('/api/queue', route(async (req) => ({ ok: true, entry: await operations.add('queue', req.body || {}) })));
  app.get('/api/not-found', route(async () => ({ items: await operations.list('notFound') })));
  app.post('/api/not-found', route(async (req) => ({ ok: true, entry: await operations.add('notFound', req.body || {}) })));
  app.delete('/api/not-found', route(async (req) => ({ ok: true, ...(await operations.remove('notFound', req.body || {})) })));
  app.get('/api/failed', route(async () => ({ items: await operations.list('failed') })));
  app.delete('/api/failed', route(async (req) => ({ ok: true, ...(await operations.remove('failed', req.body || {})) })));
